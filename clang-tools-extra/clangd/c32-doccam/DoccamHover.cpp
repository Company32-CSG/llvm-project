...
c32::markdown::Document HoverInfo::presentDoccam() const {
  ...
  auto MaybeAddNamespaceAndScopeFunc = [&]() {
    if (LocalScope.empty() && (!NamespaceScope || NamespaceScope->empty()))
      return;

    if (!Cfg.C32.Doccam.Hover.ShowScope)
      return;

    std::string WorkingScope;

    Output.heading(SectionHeadingLevel).text("Scope");

    if (NamespaceScope && !NamespaceScope->empty()) {
      WorkingScope += "namespace " +
    ...
    if (!LocalScope.empty()) {
      WorkingScope += llvm::StringRef(LocalScope).rtrim(':').str() + " {}";
    }
