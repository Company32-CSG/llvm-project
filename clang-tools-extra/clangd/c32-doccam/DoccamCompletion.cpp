...
splitAttrs(std::string_view SV)
{
	...
	while (Pos < SV.size())
	{
		...
		auto Part =
			trim(SV.substr(Pos, (std::string_view::npos == Next) ? (SV.size() - Pos) : (Next - Pos)));
...
buildTagItems(const DoccamTag& T, char INI, const Range& CompletionRange)
{
...
CodeCompleteResult
completeTags(std::string_view Contents, size_t CursorOffset, std::string_view Prefix, size_t TagPos, char INI)
{
...
bool
inDoccamComment(std::string_view Contents, size_t CursorOffset)
{
...
std::optional<CodeCompleteResult>
maybeCompleteDoccamComment(
