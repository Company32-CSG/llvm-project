...
consumeTag(ConsumeContext& Content)
{
	ParsedTag R;
...
parse(std::string_view Contents, const format::FormatStyle& Style)
{
	...
	while (auto Consumed = consumeUntil(Context, predicate::TagOrEnd))
	{
		if (!Consumed->empty())
		{
			...
			Doccam.UntaggedLines.insert(Doccam.UntaggedLines.end(), Lines.begin(), Lines.end());
		}
...
void
renderDocumentation(std::string_view Raw, c32::doccam::markdown::Document& Out)
{
	if (Raw.empty())
		return;
