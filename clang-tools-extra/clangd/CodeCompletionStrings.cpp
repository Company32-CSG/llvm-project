...
bool
isInformativeQualifierChunk(CodeCompletionString::Chunk const& Chunk)
{
...
void
appendEscapeSnippet(const llvm::StringRef Text, std::string* Out)
{
...
void
appendOptionalChunk(const CodeCompletionString& CCS, std::string* Out)
{
	for (const CodeCompletionString::Chunk& C : CCS)
	{
		switch (C.Kind)
		{
			case CodeCompletionString::CK_Optional:
				assert(C.Optional && "Expected the optional code completion string to be non-null.");
...
bool
looksLikeDocComment(llvm::StringRef CommentText)
{
...
bool
shouldPatchPlaceholder0(CodeCompletionResult::ResultKind ResultKind, CXCursorKind CursorKind)
{
...
std::string
getDocComment(const ASTContext& Ctx, const CodeCompletionResult& Result, bool CommentsFromHeaders)
{
...
std::string
getDeclComment(const ASTContext& Ctx, const NamedDecl& Decl)
{
...
void
getSignature(const CodeCompletionString& CCS, std::string* Signature, std::string* Snippet, CodeCompletionResult::ResultKind ResultKind, CXCursorKind CursorKind, bool IncludeFunctionArguments, std::string* RequiredQualifiers)
{
	...
	for (const auto& Chunk : CCS)
	{
		...
		switch (Chunk.Kind)
		{
			case CodeCompletionString::CK_TypedText:
				...
				if (!llvm::StringRef(Chunk.Text).ends_with(":"))
				{ // Treat as C++.
				...
				{ // Objective-C method with args.
					...
					{ // Subsequent argument, considered part of snippet/signature.
...
std::string
formatDocumentation(const CodeCompletionString& CCS, llvm::StringRef DocComment)
{
...
std::string
getReturnType(const CodeCompletionString& CCS)
{
	for (const auto& Chunk : CCS)
