...
namespace {
// MARK: - C32 Begin
c32::markdown::Document presentBrief(std::string contents) {
...
private:
  ...
  ScoredSignature processOverloadCandidate(const OverloadCandidate &Candidate,
    ...
    // parseDocumentation(formatDocumentation(CCS, DocComment),
    // OverloadComment);
...
CompletionItem CodeCompletion::render(const CodeCompleteOptions &Opts) const {
  ...
  if (InsertInclude || Documentation) {
    ...
    // markup::Document Doc;
    // if (InsertInclude)
    // 	Doc.addParagraph().appendText("From
    // ").appendCode(InsertInclude->Header);
    // if (Documentation)
