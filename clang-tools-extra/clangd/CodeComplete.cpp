...
const CodeCompleteOptions::CodeCompletionRankingModel CodeCompleteOptions::DefaultRankingModel = CodeCompleteOptions::DecisionForest;
#else
const CodeCompleteOptions::CodeCompletionRankingModel CodeCompleteOptions::DefaultRankingModel = CodeCompleteOptions::Heuristics;
#endif
...
CompletionItemKind
toCompletionItemKind(index::SymbolKind Kind, const llvm::StringRef* Signature = nullptr)
{
	...
	switch (Kind)
	{
		...
		case SK::Macro:
			...
			// object-like macros.
			return Signature && Signature->contains('(') ? CompletionItemKind::Function : CompletionItemKind::Constant;
		case SK::Enum:
...
CompletionItemKind
toCompletionItemKind(const CodeCompletionResult& Res, CodeCompletionContext::Kind CtxKind)
{
	if (Res.Declaration)
	...
	switch (Res.Kind)
	{
		...
		case CodeCompletionResult::RK_Macro:
			...
			return Res.MacroDefInfo && Res.MacroDefInfo->isFunctionLike() ? CompletionItemKind::Function : CompletionItemKind::Constant;
...
insertionDirective(const CodeCompleteOptions& Opts)
{
...
struct CompletionCandidate
{
	...
	size_t
	overloadSet(const CodeCompleteOptions& Opts, llvm::StringRef FileName, IncludeInserter* Inserter, CodeCompletionContext::Kind CCContextKind) const
	{
		...
		if (Inserter)
		{
			if (auto Header = headerToInsertIfAllowed(Opts, CCContextKind))
			{
				...
				{
					if (auto Spelled = Inserter->calculateIncludePath(*HeaderFile, FileName))
						HeaderForHash = *Spelled;
				}
				else
				{
					vlog("Code completion header path manipulation failed {0}", HeaderFile.takeError());
				}
			}
		}
		...
		if (IndexResult)
		{
			switch (IndexResult->SymInfo.Kind)
			{
				...
				case index::SymbolKind::Function:
					...
					return llvm::hash_combine((IndexResult->Scope + IndexResult->Name).toStringRef(Scratch), HeaderForHash);
				default:
					return 0;
			}
		}
	...
	bool
	contextAllowsHeaderInsertion(CodeCompletionContext::Kind Kind) const
	{
	...
	headerToInsertIfAllowed(const CodeCompleteOptions& Opts, CodeCompletionContext::Kind ContextKind) const
	{
		if (Opts.InsertIncludes == CodeCompleteOptions::NeverInsert || RankedIncludeHeaders.empty() || !contextAllowsHeaderInsertion(ContextKind))
			return std::nullopt;
...
using ScoredBundle = std::pair<CompletionCandidate::Bundle, CodeCompletion::Scores>;
struct ScoredBundleGreater
{
	bool
	operator()(const ScoredBundle& L, const ScoredBundle& R)
	{
		if (L.second.Total != R.second.Total)
			return L.second.Total > R.second.Total;
		return L.first.front().Name < R.first.front().Name; // Earlier name is better.
	}
};
...
{
	CodeCompletionBuilder(ASTContext* ASTCtx, const CompletionCandidate& C, CodeCompletionString* SemaCCS, llvm::ArrayRef<std::string> AccessibleScopes, const IncludeInserter& Includes, llvm::StringRef FileName, CodeCompletionContext::Kind ContextKind, const CodeCompleteOptions& Opts, bool IsUsingDeclaration, tok::TokenKind NextTokenKind)
		: ASTCtx(ASTCtx), ArgumentLists(Opts.ArgumentLists), IsUsingDeclaration(IsUsingDeclaration), NextTokenKind(NextTokenKind)
	{
		...
		if (C.SemaResult)
		{
			...
			{
				if ((C.SemaResult->Kind == CodeCompletionResult::RK_Declaration) || (C.SemaResult->Kind == CodeCompletionResult::RK_Pattern))
					if (const auto* D = C.SemaResult->getDeclaration())
						if (const auto* ND = dyn_cast<NamedDecl>(D))
							Completion.Scope = std::string(splitQualifiedName(printQualifiedName(*ND)).first);
			}
			...
			// folder.
			if (Completion.Kind == CompletionItemKind::File && Completion.Name.back() == '/')
		...
		if (C.IndexResult)
		{
			...
			if (Completion.Kind == CompletionItemKind::Missing)
				Completion.Kind = toCompletionItemKind(C.IndexResult->SymInfo.Kind, &C.IndexResult->Signature);
			...
			if (Completion.RequiredQualifier.empty() && !C.SemaResult)
			{
				...
				for (llvm::StringRef Scope : AccessibleScopes)
				{
					...
					if (Qualifier.consume_front(Scope) && Qualifier.size() < ShortestQualifier.size())
		...
		auto Inserted = [&](llvm::StringRef Header) -> llvm::Expected<std::pair<std::string, bool>>
		{
			auto ResolvedDeclaring = URI::resolve(C.IndexResult->CanonicalDeclaration.FileURI, FileName);
			...
			return std::make_pair(std::move(*Spelled), Includes.shouldInsertInclude(*ResolvedDeclaring, *ResolvedInserted));
		};
		bool					 ShouldInsert = C.headerToInsertIfAllowed(Opts, ContextKind).has_value();
		Symbol::IncludeDirective Directive	  = insertionDirective(Opts);
		...
		for (const auto& Inc : C.RankedIncludeHeaders)
		{
			...
			if (auto ToInclude = Inserted(Inc.Header))
			{
				...
				if (ToInclude->second && ShouldInsert)
					Include.Insertion = Includes.insert(ToInclude->first, Directive == Symbol::Import ? tooling::IncludeDirective::Import : tooling::IncludeDirective::Include);
	...
	void
	add(const CompletionCandidate& C, CodeCompletionString* SemaCCS, CodeCompletionContext::Kind ContextKind)
	{
		...
		{
			if (C.SemaResult)
				Completion.Deprecated &= C.SemaResult->Availability == CXAvailability_Deprecated;
			if (C.IndexResult)
				Completion.Deprecated &= bool(C.IndexResult->Flags & Symbol::Deprecated);
		}
	}
...
private:
	...
	template <bool BundledEntry::* Member>
	const bool*
	onlyValue() const
	{
		auto B = Bundled.begin(), E = Bundled.end();
	...
	summarizeSnippet() const
	{
		...
		const bool Open	 = ArgumentLists == Config::ArgumentListsPolicy::OpenDelimiter;
		const bool Delim = ArgumentLists == Config::ArgumentListsPolicy::Delimiters;
		const bool Full	 = ArgumentLists == Config::ArgumentListsPolicy::FullPlaceholders || (!None && !Open && !Delim); // <-- failsafe: Full is default
		...
		bool MayHaveArgList =
			Completion.Kind == CompletionItemKind::Function || Completion.Kind == CompletionItemKind::Method || Completion.Kind == CompletionItemKind::Constructor || Completion.Kind == CompletionItemKind::Text /*Macro*/;
		...
		if (Completion.Kind == CompletionItemKind::Interface || Completion.Kind == CompletionItemKind::Class || Completion.Kind == CompletionItemKind::Variable)
		{
			if (Snippet->front() != '<')
				return *Snippet; // Not an arg snippet?
...
SymbolID
getSymbolID(const CodeCompletionResult& R, const SourceManager& SM)
{
	switch (R.Kind)
	{
		case CodeCompletionResult::RK_Declaration:
...
struct SpecifiedScope
{
	...
	scopesForQualification()
	{
		...
		for (llvm::StringRef AS : AccessibleScopes)
			Results.insert((AS + (UnresolvedQualifier ? *UnresolvedQualifier : "")).str());
...
SpecifiedScope
getQueryScopes(CodeCompletionContext& CCContext, const Sema& CCSema, const CompletionPrefix& HeuristicPrefix, const CodeCompleteOptions& Opts)
{
	...
	const CXXScopeSpec* SemaSpecifier = CCContext.getCXXScopeSpecifier().value_or(nullptr);
	...
	if (!SemaSpecifier)
	{
		...
		{
			vlog("Sema said no scope specifier, but we saw {0} in the source code", HeuristicPrefix.Qualifier);
	...
	llvm::StringRef SpelledSpecifier = Lexer::getSourceText(CharSourceRange::getCharRange(SemaSpecifier->getRange()), CCSema.SourceMgr, clang::LangOptions());
	if (SpelledSpecifier.consume_front("::"))
		Scopes.QueryScopes = { "" };
	Scopes.UnresolvedQualifier = std::string(SpelledSpecifier);
...
bool
contextAllowsIndex(enum CodeCompletionContext::Kind K)
{
	switch (K)
	{
...
static bool
isInjectedClass(const NamedDecl& D)
{
...
static bool
isExcludedMember(const NamedDecl& D)
{
	...
	if (NameKind == DeclarationName::CXXOperatorName || NameKind == DeclarationName::CXXLiteralOperatorName || NameKind == DeclarationName::CXXConversionFunctionName)
		return true;
	return false;
}
...
{
	CompletionRecorder(const CodeCompleteOptions& Opts, llvm::unique_function<void()> ResultsCallback)
		: CodeCompleteConsumer(Opts.getClangCompleteOpts()), CCContext(CodeCompletionContext::CCC_Other), Opts(Opts), CCAllocator(std::make_shared<GlobalCodeCompletionAllocator>()), CCTUInfo(CCAllocator),
	...
	void
	ProcessCodeCompleteResults(class Sema& S, CodeCompletionContext Context, CodeCompletionResult* InResults, unsigned NumResults) final
	{
		...
		for (unsigned I = 0; I < NumResults; ++I)
		{
			...
			if (Result.Hidden && Result.Declaration && Result.Declaration->isCXXClassMember())
				continue;
			if (!Opts.IncludeIneligibleResults && (Result.Availability == CXAvailability_NotAvailable || Result.Availability == CXAvailability_NotAccessible))
				continue;
			if (Result.Declaration && !Context.getBaseType().isNull() // is this a member-access context?
			...
			if (Result.Declaration && !Context.getCXXScopeSpecifier() && isInjectedClass(*Result.Declaration))
				continue;
	...
	llvm::StringRef
	getName(const CodeCompletionResult& Result)
	{
		switch (Result.Kind)
		{
			case CodeCompletionResult::RK_Declaration:
...
// part of it.
int
paramIndexForArg(const CodeCompleteConsumer::OverloadCandidate& Candidate, int Arg)
{
...
{
public:
	SignatureHelpCollector(const clang::CodeCompleteOptions& CodeCompleteOpts, MarkupKind DocumentationFormat, const SymbolIndex* Index, SignatureHelp& SigHelp)
		: CodeCompleteConsumer(CodeCompleteOpts), SigHelp(SigHelp), Allocator(std::make_shared<clang::GlobalCodeCompletionAllocator>()), CCTUInfo(Allocator), Index(Index), DocumentationFormat(DocumentationFormat)
	{
	}

	void
	ProcessOverloadCandidates(Sema& S, unsigned CurrentArg, OverloadCandidate* Candidates, unsigned NumCandidates, SourceLocation OpenParLoc, bool Braced) override
	{
		...
		if (SrcMgr.isInMainFile(OpenParLoc))
			...
			elog("Location oustide main file in signature help: {0}", OpenParLoc.printToString(SrcMgr));
		...
		assert(CurrentArg <= (unsigned)std::numeric_limits<int>::max() && "too many arguments");
		...
		llvm::sort(ScoredSignatures, [](const ScoredSignature& L, const ScoredSignature& R)
				...
				if (L.Quality.NumberOfOptionalParameters != R.Quality.NumberOfOptionalParameters)
					return L.Quality.NumberOfOptionalParameters < R.Quality.NumberOfOptionalParameters;
...
private:
	void
	processParameterChunk(llvm::StringRef ChunkText, SignatureInformation& Signature) const
	{
	...
	void
	processOptionalChunk(const CodeCompletionString& CCS, SignatureInformation& Signature, SignatureQualitySignals& Signal) const
	{
		for (const auto& Chunk : CCS)
		{
			switch (Chunk.Kind)
			{
				case CodeCompletionString::CK_Optional:
					assert(Chunk.Optional && "Expected the optional code completion string to be non-null.");
	...
	// CompletionString.h.
	ScoredSignature
	processOverloadCandidate(const OverloadCandidate& Candidate, const CodeCompletionString& CCS, llvm::StringRef DocComment) const
	{
		SignatureInformation	Signature;
...
{
public:
	ParamNameCollector(const clang::CodeCompleteOptions& CodeCompleteOpts, std::set<std::string>& ParamNames)
		: CodeCompleteConsumer(CodeCompleteOpts), Allocator(std::make_shared<clang::GlobalCodeCompletionAllocator>()), CCTUInfo(Allocator), ParamNames(ParamNames)
	{
	}
...
void
loadMainFilePreambleMacros(const Preprocessor& PP, const PreambleData& Preamble)
{
	...
	IdentifierInfoLookup* PreambleIdentifiers = ITable.getExternalIdentifierLookup();
...
bool
semaCodeComplete(std::unique_ptr<CodeCompleteConsumer> Consumer, const clang::CodeCompleteOptions& Options, const SemaCompleteInput& Input, IncludeStructure* Includes = nullptr)
{
	...
	std::tie(FrontendOpts.CodeCompletionAt.Line, FrontendOpts.CodeCompletionAt.Column) = offsetToClangLineColumn(Input.ParseInput.Contents, Input.Offset);

	std::unique_ptr<llvm::MemoryBuffer> ContentsBuffer = llvm::MemoryBuffer::getMemBuffer(Input.ParseInput.Contents, Input.FileName);
	...
	bool		   CompletingInPreamble = Input.Offset < PreambleRegion.Size || (!PreambleRegion.PreambleEndsAtStartOfLine && Input.Offset == PreambleRegion.Size);
	if (Input.Patch)
		Input.Patch->apply(*CI);
	// NOTE: we must call BeginSourceFile after prepareCompilerInstance. Otherwise
	// the remapped buffers do not get freed.
	llvm::IntrusiveRefCntPtr<llvm::vfs::FileSystem> VFS = Input.ParseInput.TFS->view(Input.ParseInput.CompileCommand.Directory);
	if (Input.Preamble.StatCache)
		VFS = Input.Preamble.StatCache->getConsumingFS(std::move(VFS));
	auto Clang										 = prepareCompilerInstance(std::move(CI), !CompletingInPreamble ? &Input.Preamble.Preamble : nullptr, std::move(ContentsBuffer), std::move(VFS), IgnoreDiags);
	...
	{
		log("BeginSourceFile() failed when running codeComplete for {0}", Input.FileName);
		return false;
	}
	...
	{
		log("Execute() failed when running codeComplete for {0}: {1}", Input.FileName, toString(std::move(Err)));
		return false;
	}
...
bool
allowIndex(CodeCompletionContext& CC)
{
...
bool
includeSymbolFromIndex(CodeCompletionContext::Kind Kind, const Symbol& Sym)
{
	...
	if (Sym.SymInfo.Kind == index::SymbolKind::Protocol && Sym.SymInfo.Lang == index::SymbolLanguage::ObjC)
	...
	if (Kind == CodeCompletionContext::CCC_ObjCClassForwardDecl)
		return Sym.SymInfo.Kind == index::SymbolKind::Class && Sym.SymInfo.Lang == index::SymbolLanguage::ObjC;
	return true;
}
...
{
	return runAsync<std::pair<bool, SymbolSlab>>(
		[&Index, Req]()
		{
...
// source code.
FuzzyFindRequest
speculativeFuzzyFindRequestForCompletion(FuzzyFindRequest CachedReq, const CompletionPrefix& HeuristicPrefix)
{
...
std::optional<Token>
findTokenAfterCompletionPoint(SourceLocation CompletionPoint, const SourceManager& SM, const LangOptions& LangOpts)
{
	...
	Lexer TheLexer(SM.getLocForStartOfFile(LocInfo.first), LangOpts, File.begin(), TokenBegin, File.end());
	// Find the token.
	Token Tok;
...
public:
	...
	CodeCompleteFlow(PathRef FileName, const IncludeStructure& Includes, SpeculativeFuzzyFind* SpecFuzzyFind, const CodeCompleteOptions& Opts)
		: FileName(FileName), Includes(Includes), SpecFuzzyFind(SpecFuzzyFind), Opts(Opts)
	{
	}

	CodeCompleteResult
	run(const SemaCompleteInput& SemaCCInput) &&
	{
		trace::Span Tracer("CodeCompleteFlow");
		HeuristicPrefix = guessCompletionPrefix(SemaCCInput.ParseInput.Contents, SemaCCInput.Offset);
		...
		if (Opts.Index && SpecFuzzyFind && SpecFuzzyFind->CachedReq)
		{
			...
			SpecReq				  = speculativeFuzzyFindRequestForCompletion(*SpecFuzzyFind->CachedReq, HeuristicPrefix);
		...
		auto			   RecorderOwner = std::make_unique<CompletionRecorder>(Opts, [&]()
				...
				auto Style			 = getFormatStyleForFile(SemaCCInput.FileName, SemaCCInput.ParseInput.Contents, *SemaCCInput.ParseInput.TFS, false);
				const auto NextToken = findTokenAfterCompletionPoint(Recorder->CCSema->getPreprocessor().getCodeCompletionLoc(), Recorder->CCSema->getSourceManager(), Recorder->CCSema->LangOpts);
				if (NextToken)
					NextTokenKind = NextToken->getKind();
				// If preprocessor was run, inclusions from preprocessor callback should
				// already be added to Includes.
				Inserter.emplace(SemaCCInput.FileName,
				...
				auto MainFileID = Includes.getID(SM.getFileEntryForID(SM.getMainFileID()));
				assert(MainFileID);
				for (auto& HeaderIDAndDepth : Includes.includeDepth(*MainFileID))
				{
					auto& Source = ProxSources[Includes.getRealPath(HeaderIDAndDepth.getFirst())];
				...
				SPAN_ATTACH(Tracer, "sema_completion_kind", getCompletionKindString(CCContextKind));
		...
		semaCodeComplete(std::move(RecorderOwner), Opts.getClangCompleteOpts(), SemaCCInput, &Includes);
		logResults(Output, Tracer);
		return Output;
	}

	void
	logResults(const CodeCompleteResult& Output, const trace::Span& Tracer)
	{
	...
	CodeCompleteResult
	runWithoutSema(llvm::StringRef Content, size_t Offset, const ThreadsafeFS& TFS) &&
	{
		...
		SpecifiedScope Scopes;
		Scopes.QueryScopes = visibleNamespaces(Content.take_front(Offset), format::getFormattingLangOpts(Style));
		...
		{
			Scopes.QueryScopes		   = { "" };
			Scopes.UnresolvedQualifier = std::string(HeuristicPrefix.Qualifier.drop_front(2));
		}
		else
...
private:
	void
	populateContextWords(llvm::StringRef Content)
	{
		// Take last 3 lines before the completion point.
		unsigned RangeEnd = HeuristicPrefix.Qualifier.begin() - Content.data(), RangeBegin = RangeEnd;
		...
		dlog("Completion context words: {0}", llvm::join(ContextWords.keys(), ", "));
	}
	...
	runWithSema()
	{
		const auto& CodeCompletionRange = CharSourceRange::getCharRange(Recorder->CCSema->getPreprocessor().getCodeCompletionTokenRange());
		...
		{
			ReplacedRange = halfOpenToRange(Recorder->CCSema->getSourceManager(), CodeCompletionRange);
		}
		else
		{
			const auto& Pos		= sourceLocToPosition(Recorder->CCSema->getSourceManager(), Recorder->CCSema->getPreprocessor().getCodeCompletionLoc());
			ReplacedRange.start = ReplacedRange.end = Pos;
		}
		Filter				 = FuzzyMatcher(Recorder->CCSema->getPreprocessor().getCodeCompletionFilter());
		auto SpecifiedScopes = getQueryScopes(Recorder->CCContext, *Recorder->CCSema, HeuristicPrefix, Opts);
		...
		if (!QueryScopes.empty())
			ScopeProximity.emplace(QueryScopes);
		PreferredType = OpaqueType::fromType(Recorder->CCSema->getASTContext(), Recorder->CCContext.getPreferredType());
		...
		auto		IndexResults = (Opts.Index && allowIndex(Recorder->CCContext)) ? queryIndex() : SymbolSlab();
	...
	toCodeCompleteResult(const std::vector<ScoredBundle>& Scored)
	{
		...
		for (auto& C : Scored)
		{
			...
			if (Opts.Index && !Output.Completions.back().Documentation)
			{
				...
				{
					if (Cand.SemaResult && Cand.SemaResult->Kind == CodeCompletionResult::RK_Declaration)
					{
	...
	mergeResults(const std::vector<CodeCompletionResult>& SemaResults, const SymbolSlab& IndexResults, const std::vector<RawIdentifier>& IdentifierResults)
	{
		trace::Span								 Tracer("Merge and score results");
		std::vector<CompletionCandidate::Bundle> Bundles;
		llvm::DenseMap<size_t, size_t>			 BundleLookup;
		auto									 AddToBundles = [&](const CodeCompletionResult* SemaResult, const Symbol* IndexResult, const RawIdentifier* IdentifierResult)
		{
			...
			if (auto OverloadSet = C.overloadSet(Opts, FileName, Inserter ? &*Inserter : nullptr, CCContextKind))
			{
		...
		auto						  CorrespondingIndexResult = [&](const CodeCompletionResult& SemaResult) -> const Symbol*
		{
			if (auto SymID = getSymbolID(SemaResult, Recorder->CCSema->getSourceManager()))
			{
		...
		TopN<ScoredBundle, ScoredBundleGreater> Top(Opts.Limit == 0 ? std::numeric_limits<size_t>::max() : Opts.Limit);
	...
	std::optional<float>
	fuzzyScore(const CompletionCandidate& C)
	{
		// Macros can be very spammy, so we only support prefix completion.
		if (((C.SemaResult && C.SemaResult->Kind == CodeCompletionResult::RK_Macro) || (C.IndexResult && C.IndexResult->SymInfo.Kind == index::SymbolKind::Macro)) && !C.Name.starts_with_insensitive(Filter->pattern()))
			return std::nullopt;
		return Filter->match(C.Name);
	}

	CodeCompletion::Scores
	evaluateCompletion(const SymbolQualitySignals& Quality, const SymbolRelevanceSignals& Relevance)
	{
		...
		switch (Opts.RankingModel)
		{
			case RM::Heuristics:
				...
				// sound.
				Scores.ExcludingName = Relevance.NameMatch > std::numeric_limits<float>::epsilon() ? Scores.Total / Relevance.NameMatch : Scores.Quality;
				return Scores;

			case RM::DecisionForest:
				DecisionForestScores DFScores = Opts.DecisionForestScorer(Quality, Relevance, Opts.DecisionForestBase);
	...
	void
	addCandidate(TopN<ScoredBundle, ScoredBundleGreater>& Candidates, CompletionCandidate::Bundle Bundle)
	{
		...
		for (const auto& Candidate : Bundle)
		{
			if (Candidate.IndexResult)
			{
				...
				if (PreferredType && PreferredType->raw() == Candidate.IndexResult->Type)
				{
			...
			if (Candidate.SemaResult)
			{
				...
				if (PreferredType)
				{
					if (auto CompletionType = OpaqueType::fromCompletionResult(Recorder->CCSema->getASTContext(), *Candidate.SemaResult))
					{
						Relevance.HadSymbolType |= true;
		...
		if (Opts.RecordCCResult)
			Opts.RecordCCResult(toCodeCompletion(Bundle), Quality, Relevance, Scores.Total);

		dlog("CodeComplete: {0} ({1}) = {2}\n{3}{4}\n", First.Name, llvm::to_string(Origin), Scores.Total, llvm::to_string(Quality), llvm::to_string(Relevance));
	...
	toCodeCompletion(const CompletionCandidate::Bundle& Bundle)
	{
		...
		{
			CodeCompletionString* SemaCCS = Item.SemaResult ? Recorder->codeCompletionString(*Item.SemaResult) : nullptr;
			if (!Builder)
				Builder.emplace(Recorder ? &Recorder->CCSema->getASTContext() : nullptr, Item, SemaCCS, AccessibleScopes, *Inserter, FileName, CCContextKind, Opts, IsUsingDeclaration, NextTokenKind);
			else
...
)
{
	...
	if (c32::doxygen::inDoxygenComment(Content, *Offset))
	{
		const ParsedAST*					ASTPtr = nullptr;
		std::optional<ParsedAST>			BuiltAST;
		std::shared_ptr<const PreambleData> PreambleSP;

		if (nullptr != Preamble && CodeCompleteOptions::NeverParse != Opts.RunParser)
		{
			PreambleSP = std::shared_ptr<const PreambleData>(Preamble, [](const PreambleData*) {});
			clang::IgnoringDiagConsumer			DC;
			std::unique_ptr<CompilerInvocation> CI = buildCompilerInvocation(ParseInput, DC, nullptr);

			if (CI)
			{
				std::vector<Diag> EmptyDiags;

				BuiltAST = ParsedAST::build(FileName, ParseInput, std::move(CI), EmptyDiags, PreambleSP);
		...
		llvm::errs() << ">>>>>>>>>>>>>>>>>>>>> codeComplete(): In doxygen comment!\n";

		c32::doxygen::CodeCompleteArgs Ctx{
			FileName,
			ParseInput,
			Preamble,
			Opts,
		...
		auto r = c32::doxygen::completion(Ctx);

		llvm::errs() << ">>>>>>>>>>>>>>>>>>>>> codeComplete(): c32::doxygen::completion() returned " << r.Completions.size() << " results\n";
	...
	return (!Preamble || Opts.RunParser == CodeCompleteOptions::NeverParse)
		? std::move(Flow).runWithoutSema(ParseInput.Contents, *Offset, *ParseInput.TFS)
...
SignatureHelp
signatureHelp(PathRef FileName, Position Pos, const PreambleData& Preamble, const ParseInputs& ParseInput, MarkupKind DocumentationFormat)
{
	auto Offset = positionToOffset(ParseInput.Contents, Pos);
	if (!Offset)
	{
	...
	semaCodeComplete(
		std::make_unique<SignatureHelpCollector>(Options, DocumentationFormat, ParseInput.Index, Result),
		Options,
		{ FileName,
		  *Offset,
		  Preamble,
...
bool
isIndexedForCodeCompletion(const NamedDecl& ND, ASTContext& ASTCtx)
{
...
CompletionItem
CodeCompletion::render(const CodeCompleteOptions& Opts) const
{
	CompletionItem LSP;
	...
	LSP.label = ((InsertInclude && InsertInclude->Insertion) ? Opts.IncludeIndicator.Insert : Opts.IncludeIndicator.NoInsert) + (Opts.ShowOrigins ? "[" + llvm::to_string(Origin) + "]" : "") + RequiredQualifier + Name;
	LSP.labelDetails.emplace();
	LSP.labelDetails->detail = Signature;

	LSP.kind	   = Kind;
	LSP.detail	   = BundleSize > 1 ? std::string(llvm::formatv("[{0} overloads]", BundleSize)) : ReturnType;
	...
	LSP.insertTextFormat = (Opts.EnableSnippets && !SnippetSuffix.empty()) ? InsertTextFormat::Snippet : InsertTextFormat::PlainText;
...
llvm::raw_ostream&
operator<<(llvm::raw_ostream& OS, const CodeCompletion& C)
{
...
llvm::raw_ostream&
operator<<(llvm::raw_ostream& OS, const CodeCompleteResult& R)
{
	OS << "CodeCompleteResult: " << R.Completions.size() << (R.HasMore ? "+" : "") << " (" << getCompletionKindString(R.Context) << ")"
	   << " items:\n";
...
bool
isIncludeFile(llvm::StringRef Line)
{
	Line = Line.ltrim();
	if (!Line.consume_front("#"))
		return false;
	Line = Line.ltrim();
	if (!(Line.consume_front("include_next") || Line.consume_front("include") || Line.consume_front("import")))
		return false;
	Line = Line.ltrim();
	if (Line.consume_front("<"))
		return Line.count('>') == 0;
...
bool
allowImplicitCompletion(llvm::StringRef Content, unsigned Offset)
{
	...
	if ((Content.ends_with("<") || Content.ends_with("\"") || Content.ends_with("/")) && isIncludeFile(Content))
		return true;

	// Complete words. Give non-ascii characters the benefit of the doubt.
	return !Content.empty() && (isAsciiIdentifierContinue(Content.back()) || !llvm::isASCII(Content.back()));
}

CodeCompleteResult
codeCompleteFlowHook(
	PathRef				  FileName,
	size_t				  Offset,
	const PreambleData*	  Preamble,
	const ParseInputs&	  ParseInput,
...
)
{
	...
	auto Patch = PreamblePatch::createFullPatch(FileName, ParseInputPatch, *Preamble);

	return (!Preamble || Opts.RunParser == CodeCompleteOptions::NeverParse)
		? std::move(Flow).runWithoutSema(ParseInputPatch.Contents, Offset, *ParseInputPatch.TFS)
		: std::move(Flow).run({ FileName, Offset, *Preamble, Patch, ParseInputPatch });
}
