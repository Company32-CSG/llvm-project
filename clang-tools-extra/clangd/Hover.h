...
/// embedding clients can use the structured information to provide
/// their own UI.
struct HoverInfo
{
	...
	struct PrintedType
	{
		...
		PrintedType(const char* Type, const char* AKAType) : Type(Type), AKA(AKAType) {}
	...
	struct Param
	{
		...
		/// TemplateParameters), might be std::nullopt for macro
		/// parameters.
	...
	/// For a variable named Bar, declared in clang::clangd::Foo::getFoo
	/// the following fields will hold:
	...
	/// Remaining named contexts in symbol's qualified name, empty
	/// string means symbol is not local.
	...
	/// Access specifier for declarations inside class/struct/unions,
	/// empty for others.
	...
	/// Contains the bit-size of fields and types where it's
	/// interesting.
	...
	/// Contains the padding following a field within the enclosing
	/// class.
	std::optional<uint64_t> Padding;

	/// Contains the alignment of fields and types where it's
	/// interesting.
	std::optional<uint64_t> Align;

	// Set when symbol is inside function call. Contains information
	...
	struct PassType
	{
		...
		// True if type conversion happened. This includes calls to
		// implicit constructor, as well as built-in type conversions.
		// Casting to base class is not considered conversion.
...
inline bool
operator==(const HoverInfo::PrintedType& LHS, const HoverInfo::PrintedType& RHS)
{
	return std::tie(LHS.Type, LHS.AKA) == std::tie(RHS.Type, RHS.AKA);
}

inline bool
operator==(const HoverInfo::PassType& LHS, const HoverInfo::PassType& RHS)
{
	return std::tie(LHS.PassBy, LHS.Converted) == std::tie(RHS.PassBy, RHS.Converted);
}
...
llvm::raw_ostream& operator<<(llvm::raw_ostream&, const HoverInfo::PrintedType&);
llvm::raw_ostream& operator<<(llvm::raw_ostream&, const HoverInfo::Param&);

inline bool
operator==(const HoverInfo::Param& LHS, const HoverInfo::Param& RHS)
{
	return std::tie(LHS.Type, LHS.Name, LHS.Default) == std::tie(RHS.Type, RHS.Name, RHS.Default);
}
