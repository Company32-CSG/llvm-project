...
static constexpr trace::Metric TweakAvailable("tweak_available", trace::Metric::Counter, "tweak_id");

// Update the FileIndex with new ASTs and plumb the diagnostics responses.
struct UpdateIndexCallbacks : public ParsingCallbacks
{
	UpdateIndexCallbacks(FileIndex* FIndex, ClangdServer::Callbacks* ServerCallbacks, const ThreadsafeFS& TFS, AsyncTaskRunner* Tasks, bool CollectInactiveRegions)
		: FIndex(FIndex), ServerCallbacks(ServerCallbacks), TFS(TFS), Stdlib{ std::make_shared<StdLibSet>() }, Tasks(Tasks), CollectInactiveRegions(CollectInactiveRegions)
	{
	}

	void
	onPreambleAST(PathRef Path, llvm::StringRef Version, CapturedASTCtx ASTCtx, std::shared_ptr<const include_cleaner::PragmaIncludes> PI) override
	{
		...
		auto Task = [FIndex(FIndex), Path(Path.str()), Version(Version.str()), ASTCtx(std::move(ASTCtx)), PI(std::move(PI))]() mutable
		{
			trace::Span Tracer("PreambleIndexing");
			FIndex->updatePreamble(Path, Version, ASTCtx.getASTContext(), ASTCtx.getPreprocessor(), *PI);
		};

		if (Tasks)
		{
			Tasks->runAsync("Preamble indexing for:" + Path + Version, std::move(Task));
		}
		else
			Task();
	}
	...
	void
	indexStdlib(const CompilerInvocation& CI, StdLibLocation Loc)
	{
	...
	void
	onMainAST(PathRef Path, ParsedAST& AST, PublishFn Publish) override
	{
		if (FIndex)
		...
		if (ServerCallbacks)
			Publish(
				[&]()
				{
					ServerCallbacks->onDiagnosticsReady(Path, AST.version(), AST.getDiagnostics());
	...
	void
	onFileUpdated(PathRef File, const TUStatus& Status) override
	{
		if (ServerCallbacks)
	...
	void
	onPreamblePublished(PathRef File) override
	{
		if (ServerCallbacks)
...
class DraftStoreFS : public ThreadsafeFS
{
  ...
  private:
	...
	{
		auto OFS = llvm::makeIntrusiveRefCnt<llvm::vfs::OverlayFileSystem>(Base.view(std::nullopt));
...
ClangdServer::ClangdServer(const GlobalCompilationDatabase& CDB, const ThreadsafeFS& TFS, const Options& Opts, Callbacks* Callbacks)
	: FeatureModules(Opts.FeatureModules), CDB(CDB), TFS(TFS), DynamicIdx(Opts.BuildDynamicSymbolIndex ? new FileIndex(Opts.EnableOutgoingCalls) : nullptr), ModulesManager(Opts.ModulesManager),
	  ClangTidyProvider(Opts.ClangTidyProvider), UseDirtyHeaders(Opts.UseDirtyHeaders), LineFoldingOnly(Opts.LineFoldingOnly), PreambleParseForwardingFunctions(Opts.PreambleParseForwardingFunctions),
	  ImportInsertions(Opts.ImportInsertions), PublishInactiveRegions(Opts.PublishInactiveRegions), WorkspaceRoot(Opts.WorkspaceRoot),
	  Transient(Opts.ImplicitCancellation ? TUScheduler::InvalidateOnUpdate : TUScheduler::NoInvalidation), DirtyFS(std::make_unique<DraftStoreFS>(TFS, DraftMgr))
{
	...
	// is parsed.
	WorkScheduler.emplace(CDB, TUScheduler::Options(Opts), std::make_unique<UpdateIndexCallbacks>(DynamicIdx.get(), Callbacks, TFS, IndexTasks ? &*IndexTasks : nullptr, PublishInactiveRegions));
...
void
ClangdServer::reparseOpenFilesIfNeeded(llvm::function_ref<bool(llvm::StringRef File)> Filter)
{
	// Reparse only opened files that were modified.
	for (const Path& FilePath : DraftMgr.getActiveFiles())
		if (Filter(FilePath))
			if (auto Draft = DraftMgr.getDraft(FilePath)) // else disappeared in race?
				addDocument(FilePath, *Draft->Contents, Draft->Version, WantDiagnostics::Auto);
}

std::shared_ptr<const std::string>
ClangdServer::getDraft(PathRef File) const
{
...
ClangdServer::createConfiguredContextProvider(const config::Provider* Provider, Callbacks* Publish)
{
	if (!Provider)
		...
		operator()(llvm::StringRef File)
		{
			...
			if (!ReportableDiagnostics.empty())
			{
				...
				for (auto& Entry : ReportableDiagnostics)
					Publish->onDiagnosticsReady(Entry.first(), /*Version=*/"", Entry.second);
			}
			return Context::current().derive(Config::Key, std::move(C));
		}

		void
		handleDiagnostic(const llvm::SMDiagnostic& D, std::vector<Diag>* ClientDiagnostics)
		{
			switch (D.getKind())
			{
				case llvm::SourceMgr::DK_Error:
					elog("config error at {0}:{1}:{2}: {3}", D.getFilename(), D.getLineNo(), D.getColumnNo(), D.getMessage());
					break;
				case llvm::SourceMgr::DK_Warning:
					log("config warning at {0}:{1}:{2}: {3}", D.getFilename(), D.getLineNo(), D.getColumnNo(), D.getMessage());
					break;
				case llvm::SourceMgr::DK_Note:
				case llvm::SourceMgr::DK_Remark:
					vlog("config note at {0}:{1}:{2}: {3}", D.getFilename(), D.getLineNo(), D.getColumnNo(), D.getMessage());
...
void
ClangdServer::removeDocument(PathRef File)
{
...
void
ClangdServer::codeComplete(PathRef File, Position Pos, const clangd::CodeCompleteOptions& Opts, Callback<CodeCompleteResult> CB)
{
	...
	auto Task = [Pos, CodeCompleteOpts, File = File.str(), CB = std::move(CB), this](llvm::Expected<InputsAndPreamble> IP) mutable
	{
		if (!IP)
...
void
ClangdServer::signatureHelp(PathRef File, Position Pos, MarkupKind DocumentationFormat, Callback<SignatureHelp> CB)
{

	auto Action = [Pos, File = File.str(), CB = std::move(CB), DocumentationFormat, this](llvm::Expected<InputsAndPreamble> IP) mutable
	{
		if (!IP)
			return CB(IP.takeError());
		...
		if (!IP->Contents.ends_with("\n"))
			ParseInput.Contents.append("\n");
		ParseInput.Index = Index;
		CB(clangd::signatureHelp(File, Pos, *PreambleData, ParseInput, DocumentationFormat));
	};
...
void
ClangdServer::formatOnType(PathRef File, Position Pos, StringRef TriggerText, Callback<std::vector<TextEdit>> CB)
{
	auto Code = getDraft(File);
	if (!Code)
		return CB(llvm::make_error<LSPError>("trying to format non-added document", ErrorCode::InvalidParams));
...
void
ClangdServer::prepareRename(PathRef File, Position Pos, std::optional<std::string> NewName, const RenameOptions& RenameOpts, Callback<RenameResult> CB)
{
	auto Action = [Pos, File = File.str(), CB = std::move(CB), NewName = std::move(NewName), RenameOpts](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
...
void
ClangdServer::rename(PathRef File, Position Pos, llvm::StringRef NewName, const RenameOptions& Opts, Callback<RenameResult> CB)
{
	auto Action = [File = File.str(), NewName = NewName.str(), Pos, Opts, CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		// Tracks number of files edited per invocation.
		static constexpr trace::Metric RenameFiles("rename_files", trace::Metric::Distribution);
		if (!InpAST)
			return CB(InpAST.takeError());
		auto R = clangd::rename({ Pos, NewName, InpAST->AST, File, DirtyFS->view(std::nullopt), Index, Opts });
		if (!R)
...
tweakSelection(const Range& Sel, const InputsAndAST& AST, llvm::vfs::FileSystem* FS)
{
...
{
	bool IsClangTidyRename = Diag->Source == Diag::ClangTidy && Diag->Name == "readability-identifier-naming" && !Fix.Edits.empty();
...
void
ClangdServer::codeAction(const CodeActionInputs& Params, Callback<CodeActionResult> CB)
{
	auto Action = [Params, CB = std::move(CB), FeatureModules(this->FeatureModules)](Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
		auto KindAllowed = [Only(Params.RequestedActionKinds)](llvm::StringRef Kind)
		{
			if (Only.empty())
				return true;
	...
	WorkScheduler->runWithAST("codeAction", Params.File, std::move(Action), Transient);
}

void
ClangdServer::applyTweak(PathRef File, Range Sel, StringRef TweakID, Callback<Tweak::Effect> CB)
{
	// Tracks number of times a tweak has been attempted.
	static constexpr trace::Metric TweakAttempt("tweak_attempt", trace::Metric::Counter, "tweak_id");
	// Tracks number of times a tweak has failed to produce edits.
	static constexpr trace::Metric TweakFailed("tweak_failed", trace::Metric::Counter, "tweak_id");
	TweakAttempt.record(1, TweakID);
	auto Action = [File = File.str(), Sel, TweakID = TweakID.str(), CB = std::move(CB), this](Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::locateSymbolAt(PathRef File, Position Pos, Callback<std::vector<LocatedSymbol>> CB)
{
	auto Action = [Pos, CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::switchSourceHeader(PathRef Path, Callback<std::optional<clangd::Path>> CB)
{
	...
	if (auto CorrespondingFile = getCorrespondingHeaderOrSource(Path, TFS.view(std::nullopt)))
		return CB(std::move(CorrespondingFile));
	auto Action = [Path = Path.str(), CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::findDocumentHighlights(PathRef File, Position Pos, Callback<std::vector<DocumentHighlight>> CB)
{
	auto Action = [Pos, CB = std::move(CB)](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::findHover(PathRef File, Position Pos, Callback<std::optional<HoverInfo>> CB)
{
	auto Action = [File = File.str(), Pos, CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());

		format::FormatStyle Style = getFormatStyleForFile(File, InpAST->Inputs.Contents, *InpAST->Inputs.TFS, false);
...
void
ClangdServer::typeHierarchy(PathRef File, Position Pos, int Resolve, TypeHierarchyDirection Direction, Callback<std::vector<TypeHierarchyItem>> CB)
{
	auto Action = [File = File.str(), Pos, Resolve, Direction, CB = std::move(CB), this](Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
		CB(clangd::getTypeHierarchy(InpAST->AST, Pos, Resolve, Direction, Index, File));
	};
...
void
ClangdServer::prepareCallHierarchy(PathRef File, Position Pos, Callback<std::vector<CallHierarchyItem>> CB)
{
	auto Action = [File = File.str(), Pos, CB = std::move(CB)](Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::inlayHints(PathRef File, std::optional<Range> RestrictRange, Callback<std::vector<InlayHint>> CB)
{
	auto Action = [RestrictRange(std::move(RestrictRange)), CB = std::move(CB)](Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::onFileEvent(const DidChangeWatchedFilesParams& Params)
{
...
void
ClangdServer::documentSymbols(llvm::StringRef File, Callback<std::vector<DocumentSymbol>> CB)
{
	...
	WorkScheduler->runWithAST("DocumentSymbols", File, std::move(Action), Transient);
}

void
ClangdServer::foldingRanges(llvm::StringRef File, Callback<std::vector<FoldingRange>> CB)
{
	auto Code = getDraft(File);
	if (!Code)
		return CB(llvm::make_error<LSPError>("trying to compute folding ranges for non-added document", ErrorCode::InvalidParams));
	auto Action = [LineFoldingOnly = LineFoldingOnly, CB = std::move(CB), Code = std::move(*Code)]() mutable
	{
...
void
ClangdServer::findType(llvm::StringRef File, Position Pos, Callback<std::vector<LocatedSymbol>> CB)
{
	auto Action = [Pos, CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
...
void
ClangdServer::findImplementations(PathRef File, Position Pos, Callback<std::vector<LocatedSymbol>> CB)
{
	auto Action = [Pos, CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
...
void
ClangdServer::findReferences(PathRef File, Position Pos, uint32_t Limit, bool AddContainer, Callback<ReferencesResult> CB)
{
	auto Action = [Pos, Limit, AddContainer, CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::symbolInfo(PathRef File, Position Pos, Callback<std::vector<SymbolDetails>> CB)
{
	auto Action = [Pos, CB = std::move(CB)](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
...
void
ClangdServer::semanticRanges(PathRef File, const std::vector<Position>& Positions, Callback<std::vector<SelectionRange>> CB)
{
	auto Action = [Positions, CB = std::move(CB)](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
...
void
ClangdServer::documentLinks(PathRef File, Callback<std::vector<DocumentLink>> CB)
{
	auto Action = [CB = std::move(CB)](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
	...
	WorkScheduler->runWithAST("DocumentLinks", File, std::move(Action), Transient);
}

void
ClangdServer::semanticHighlights(PathRef File, Callback<std::vector<HighlightingToken>> CB)
{

	auto Action = [CB = std::move(CB), PublishInactiveRegions = PublishInactiveRegions](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
			return CB(InpAST.takeError());
	...
	WorkScheduler->runWithAST("SemanticHighlights", File, std::move(Action), Transient);
}

void
ClangdServer::getAST(PathRef File, std::optional<Range> R, Callback<std::optional<ASTNode>> CB)
{
	auto Action = [R, CB(std::move(CB))](llvm::Expected<InputsAndAST> Inputs) mutable
	{
		if (!Inputs)
		...
		if (!R)
		{
			...
			auto Node = DynTypedNode::create(*Inputs->AST.getASTContext().getTranslationUnitDecl());
...
void
ClangdServer::customAction(PathRef File, llvm::StringRef Name, Callback<InputsAndAST> Action)
{
	WorkScheduler->runWithAST(Name, File, std::move(Action));
}

void
ClangdServer::diagnostics(PathRef File, Callback<std::vector<Diag>> CB)
{
	auto Action = [CB = std::move(CB)](llvm::Expected<InputsAndAST> InpAST) mutable
	{
		if (!InpAST)
...
llvm::StringMap<TUScheduler::FileStats>
...
ClangdServer::blockUntilIdleForTest(std::optional<double> TimeoutSeconds)
{
...
#if defined(__has_feature) && (__has_feature(address_sanitizer) || __has_feature(hwaddress_sanitizer) || __has_feature(memory_sanitizer) || __has_feature(thread_sanitizer))
	...
	for (std::optional<double> Timeout : { TimeoutSeconds, TimeoutSeconds, std::optional<double>(0) })
	{
	...
	assert(WorkScheduler->blockUntilIdle(Deadline::zero()) && "Something scheduled work while we're blocking the main thread!");
	return true;
}

void
ClangdServer::profile(MemoryTree& MT) const
{
	if (DynamicIdx)
