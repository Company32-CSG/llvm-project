...
bool
looksLikeTag(llvm::StringRef Contents)
{
	if (Contents.empty())
		return false;
	...
	Contents = Contents.drop_while([](char C) { return llvm::isAlnum(C) || C == '-' || C == '_' || C == ':'; }).drop_while(llvm::isSpace);
...
bool
needsLeadingEscape(char C, llvm::StringRef Before, llvm::StringRef After, bool StartsLine)
{
	...
	auto SpaceSurrounds = [&]()
	{
		return (After.empty() || llvm::isSpace(After.front())) && (Before.empty() || llvm::isSpace(Before.back()));
	};

	auto WordSurrounds = [&]()
	{
		return (!After.empty() && llvm::isAlnum(After.front())) && (!Before.empty() && llvm::isAlnum(Before.back()));
	};
	...
	switch (C)
	{
		...
		case ')':
			return StartsLine && !Before.empty() && llvm::all_of(Before, llvm::isDigit) && After.starts_with(" ");
		default:
			return false;
	}
}
...
std::string
renderText(llvm::StringRef Input, bool StartsLine)
{
	std::string R;
...
std::string
renderBlocks(llvm::ArrayRef<std::unique_ptr<Block>> Children, void (Block::*RenderFunc)(llvm::raw_ostream&) const)
{
	std::string R;
	...
	llvm::copy_if(TrimmedText,
		std::back_inserter(AdjustedResult),
		[&TrimmedText](const char& C)
		{
			return !llvm::StringRef(TrimmedText.data(), &C - TrimmedText.data() + 1)
...
class Ruler : public Block
{
  public:
	void
	renderMarkdown(llvm::raw_ostream& OS) const override
	{
	...
	void
	renderPlainText(llvm::raw_ostream& OS) const override
	{
		OS << '\n';
	}
...
class CodeBlock : public Block
{
  public:
	...
	CodeBlock(std::string Contents, std::string Language) : Contents(std::move(Contents)), Language(std::move(Language)) {}
...
void
Paragraph::renderMarkdown(llvm::raw_ostream& OS) const
{
...
llvm::StringRef
chooseMarker(llvm::ArrayRef<llvm::StringRef> Options, llvm::StringRef Text)
{
...
void
Paragraph::renderPlainText(llvm::raw_ostream& OS) const
{
	bool NeedsSpace = false;
	for (auto& C : Chunks)
	{
...
void
BulletList::renderMarkdown(llvm::raw_ostream& OS) const
{
	for (auto& D : Items)
	{
...
void
BulletList::renderPlainText(llvm::raw_ostream& OS) const
{
	for (auto& D : Items)
	{
		// Instead of doing this we might prefer passing Indent to children to get
...
void
Document::addCodeBlock(std::string Code, std::string Language)
{
