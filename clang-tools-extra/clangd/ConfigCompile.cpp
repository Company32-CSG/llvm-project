...
llvm::StringRef
configRelative(llvm::StringRef Path, llvm::StringRef FragmentDir)
{
...
struct CompiledFragmentImpl
{
	...
	bool
	operator()(const Params& P, Config& C) const
	{
...
{
	FragmentCompiler(CompiledFragmentImpl& Out, DiagnosticCallback D, llvm::SourceMgr* SM)
	...
	compileRegex(const Located<std::string>& Text, llvm::Regex::RegexFlags Flags = llvm::Regex::NoFlags)
	{
	...
	makeAbsolute(Located<std::string> Path, llvm::StringLiteral Description, llvm::sys::path::Style Style)
	{
		if (llvm::sys::path::is_absolute(*Path))
			return *Path;
		if (FragmentDirectory.empty())
		{
			diag(Error, llvm::formatv("{0} must be an absolute path, because this fragment is not "
	...
	public:
		EnumSwitch(llvm::StringRef EnumName, const Located<std::string>& In, FragmentCompiler& Outer)
			: Outer(Outer), EnumName(EnumName), Input(In) {}

		EnumSwitch&
		map(llvm::StringLiteral Name, T Value)
		{
		...
		value()
		{
			if (!Result)
				Outer.diag(
					Warning,
					llvm::formatv("Invalid {0} value '{1}'. Valid values are {2}.", EnumName, *Input, llvm::join(ValidValues, ", "))
						.str(),
					Input.Range
				);
			return Result;
	...
	template <typename T>
	EnumSwitch<T>
	compileEnum(llvm::StringRef EnumName, const Located<std::string>& In)
	{
	...
	void
	compile(Fragment::CompileFlagsBlock&& F)
	{
		if (F.Compiler)
			Out.Apply.push_back(
		...
		if (!F.Remove.empty())
		{
			...
			for (auto& A : F.Remove)
				Remove->strip(*A);
			Out.Apply.push_back([Remove(std::shared_ptr<const ArgStripper>(std::move(Remove)))](const Params&, Config& C)
								{ C.CompileFlags.Edits.push_back(
		...
		if (F.CompilationDatabase)
		{
			...
			{
				if (auto Path =
						makeAbsolute(*F.CompilationDatabase, "CompilationDatabase", llvm::sys::path::Style::native))
				{
	...
	void
	compile(Fragment::IndexBlock::ExternalBlock&& External, llvm::SMRange BlockRange)
	{
		if (External.Server && !Trusted)
		{
			diag(Error, "Remote index may not be specified by untrusted configuration. "
		...
		if (SourceCount != 1)
		{
			diag(Error, "Exactly one of File, Server or None must be set.", BlockRange);
			return;
		}
		...
		else if (External.File)
		{
			...
			auto AbsPath = makeAbsolute(std::move(*External.File), "File", llvm::sys::path::Style::native);
			if (!AbsPath)
				return;
		...
		if (Spec.Kind != Config::ExternalIndexSpec::None)
		{
			...
			auto AbsPath = makeAbsolute(std::move(*External.MountPoint), "MountPoint", llvm::sys::path::Style::posix);
			if (!AbsPath)
				return;
	...
	void
	compile(Fragment::DiagnosticsBlock&& F)
	{
		...
		if (F.UnusedIncludes)
		{
			auto Val = compileEnum<Config::IncludesPolicy>("UnusedIncludes", **F.UnusedIncludes)
		...
		if (F.MissingIncludes)
			if (auto Val = compileEnum<Config::IncludesPolicy>("MissingIncludes", **F.MissingIncludes)
							   .map("Strict", Config::IncludesPolicy::Strict)
							   .map("None", Config::IncludesPolicy::None)
							   .value())
				Out.Apply.push_back([Val](const Params&, Config& C)
		...
		if (!F.FullyQualifiedNamespaces.empty())
		{
			...
			Out.Apply.push_back([FullyQualifiedNamespaces(
								...
								{ C.Style.FullyQualifiedNamespaces.insert(
									  ...
									  FullyQualifiedNamespaces.begin(),
									  FullyQualifiedNamespaces.end()
								  ); });
		}
	...
	auto
	compileHeaderRegexes(llvm::ArrayRef<Located<std::string>> HeaderPatterns)
		-> std::optional<std::function<bool(llvm::StringRef)>>
	{
		...
		for (auto& HeaderPattern : HeaderPatterns)
		{
			...
			{
				diag(Warning, llvm::formatv("Invalid regular expression '{0}': {1}", *HeaderPattern, RegexError).str(), HeaderPattern.Range);
				continue;
			}
	...
	void
	appendTidyCheckSpec(std::string& CurSpec, const Located<std::string>& Arg, bool IsPositive)
	{
		...
		if (!Str.contains('*'))
		{
			...
			{
				diag(Warning, llvm::formatv("clang-tidy check '{0}' was not found", Str).str(), Arg.Range);
				return;
			}
			auto Fast = isFastTidyCheck(Str);
			if (!Fast.has_value())
			{
				diag(Warning, llvm::formatv("Latency of clang-tidy check '{0}' is not known. "
			...
			{
				diag(Warning, llvm::formatv("clang-tidy check '{0}' is slow. "
	...
	void
	compile(Fragment::DiagnosticsBlock::ClangTidyBlock&& F)
	{
		std::string Checks;
		...
		if (!F.CheckOptions.empty())
		{
			...
			for (auto& Opt : F.CheckOptions)
				CheckOptions.emplace_back(std::move(*Opt.first), std::move(*Opt.second));
			Out.Apply.push_back(
		...
		if (F.FastCheckFilter.has_value())
			if (auto Val = compileEnum<Config::FastCheckPolicy>("FastCheckFilter", *F.FastCheckFilter)
	...
	void
	compile(Fragment::DiagnosticsBlock::IncludesBlock&& F)
	{
#ifdef CLANGD_PATH_CASE_INSENSITIVE
		static llvm::Regex::RegexFlags Flags = llvm::Regex::IgnoreCase;
#else
	...
	void
	compile(Fragment::CompletionBlock&& F)
	{
		if (F.AllScopes)
		{
			Out.Apply.push_back(
		...
		{
			if (auto Val =
					compileEnum<Config::ArgumentListsPolicy>("ArgumentLists", *F.ArgumentLists)
						.map("None", Config::ArgumentListsPolicy::None)
						.map("OpenDelimiter", Config::ArgumentListsPolicy::OpenDelimiter)
						.map("Delimiters", Config::ArgumentListsPolicy::Delimiters)
						.map("FullPlaceholders", Config::ArgumentListsPolicy::FullPlaceholders)
						.value())
				Out.Apply.push_back([Val](const Params&, Config& C)
	...
	void
	compile(Fragment::InlayHintsBlock&& F)
	{
		if (F.Enabled)
	...
	void
	compile(Fragment::SemanticTokensBlock&& F)
	{
	...
	void
	compile(Fragment::C32Block&& F)
	{
		compile(std::move(F.Hover));
	}

	void
	compile(Fragment::C32Block::HoverBlock&& F)
	{
		if (F.ShowHoveringOver)
			Out.Apply.push_back([Value(**F.ShowHoveringOver)](const Params&, Config& C)
		...
		if (F.ShowSizeAndOffset)
			Out.Apply.push_back([Value(**F.ShowSizeAndOffset)](const Params&, Config& C)
								{ C.C32.Hover.ShowSizeAndOffset = Value; });

		if (F.ShowProvidedSymbols)
			Out.Apply.push_back([Value(**F.ShowProvidedSymbols)](const Params&, Config& C)
								{ C.C32.Hover.ShowProvidedSymbols = Value; });

		if (F.ShowCalleeInfo)
			Out.Apply.push_back([Value(**F.ShowCalleeInfo)](const Params&, Config& C)
	...
	void
	diag(llvm::SourceMgr::DiagKind Kind, llvm::StringRef Message, llvm::SMRange Range)
	{
...
Fragment::compile(DiagnosticCallback D) &&
{
	...
	vlog("Config fragment: compiling {0}:{1} -> {2} (trusted={3})", ConfigFile, LineCol.first, Result.get(), Source.Trusted);
