...
class LSPError : public llvm::ErrorInfo<LSPError>
{
  public:
	...
	void
	log(llvm::raw_ostream& OS) const override
	{
		OS << int(Code) << ": " << Message;
	}
	std::error_code
...
struct URIForFile
{
	...
	friend bool
	operator==(const URIForFile& LHS, const URIForFile& RHS)
	{
...
inline bool
operator==(const TextEdit& L, const TextEdit& R)
{
...
inline bool
fromJSON(const llvm::json::Value&, NoParams&, llvm::json::Path)
{
	return true;
}
...
struct InitializeParams
{
	...
	/// The name and version of the client editor.
...
template <typename T>
llvm::json::Value
toJSON(const ProgressParams<T>& P)
{
...
struct DocumentHighlight
{
	...
	friend bool
	operator<(const DocumentHighlight& LHS, const DocumentHighlight& RHS)
	{
...
struct InlayHintLabelPart
{
	...
	InlayHintLabelPart(std::string value, std::optional<Location> location = std::nullopt) : value(std::move(value)), location(std::move(location)) {}
...
struct DocumentLink
{
	...
	friend bool
	operator==(const DocumentLink& LHS, const DocumentLink& RHS)
	{
...
template <> struct DenseMapInfo<clang::clangd::Range>
{
	...
	static unsigned
	getHashValue(const Range& Val)
	{
		return llvm::hash_combine(Val.start.line, Val.start.character, Val.end.line, Val.end.character);
	}
	static bool
	isEqual(const Range& LHS, const Range& RHS)
	{
		return std::tie(LHS.start, LHS.end) == std::tie(RHS.start, RHS.end);
	}
};
