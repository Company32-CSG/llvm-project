...
/// Given a declaration \p D, return a human-readable string
/// representing the local scope in which it is declared, i.e.
/// class(es) and method name. Returns an empty string if it is not
/// local.
std::string
...
/// Returns the human-readable representation for namespace
/// containing the declaration \p D. Returns empty if it is
/// contained global namespace.
std::string
getNamespaceScope(const Decl* D)
{
	const DeclContext* DC = D->getDeclContext();

	// ObjC does not have the concept of namespaces, so instead we
	// support local scopes.
...
printDefinition(const Decl* D, PrintingPolicy PP, const syntax::TokenBuffer& TB)
{
	if (auto* VD = llvm::dyn_cast<VarDecl>(D))
	{
		...
		{
			// Initializers might be huge and result in lots of
			// memory allocations in some catastrophic cases. Such
			// long lists are not useful in hover cards anyway.
...
const char*
getMarkdownLanguage(const ASTContext& Ctx)
{
...
printType(QualType QT, ASTContext& ASTCtx, const PrintingPolicy& PP)
{
	// TypePrinter doesn't resolve decltypes, so resolve them here.
	// FIXME: This doesn't handle composite types that contain a
	// decltype in them. We should rather have a printing policy for
	// that.
	...
	// Special case: if the outer type is a tag type without
	// qualifiers, then include the tag for extra clarity. This
	// isn't very idiomatic, so don't attempt it for complex cases,
	// including pointers/references, template specializations, etc.
...
HoverInfo::PrintedType
printType(const TemplateTypeParmDecl* TTP)
{
	HoverInfo::PrintedType Result;
...
HoverInfo::PrintedType
printType(const NonTypeTemplateParmDecl* NTTP, const PrintingPolicy& PP)
{
...
printType(const TemplateTemplateParmDecl* TTP, const PrintingPolicy& PP)
{
	...
	// FIXME: TemplateTemplateParameter doesn't store the info on
	// whether this param was a "typename" or "class".
	OS << "> class";
	return Result;
}

std::vector<HoverInfo::Param>
fetchTemplateParameters(const TemplateParameterList* Params, const PrintingPolicy& PP)
{
	assert(Params);
...
const FunctionDecl*
getUnderlyingFunction(const Decl* D)
{
...
getDeclForComment(const NamedDecl* D)
{
	...
	{
		// Template may not be instantiated e.g. if the type didn't
		// need to be complete; fallback to primary template.
	...
	else if (const auto* TSD = llvm::dyn_cast<VarTemplateSpecializationDecl>(D))
	{
		if (TSD->getTemplateSpecializationKind() == TSK_Undeclared)
			DeclForComment = TSD->getSpecializedTemplate();
		else if (const auto* TIP = TSD->getTemplateInstantiationPattern())
	...
	// getDeclForComment(X). This is usually not needed, but in
	// strange cases of comparision operators being instantiated
	// from spasceship operater, which itself is a template
...
void
enhanceFromIndex(HoverInfo& Hover, const NamedDecl& ND, const SymbolIndex* Index)
{
	assert(&ND == getDeclForComment(&ND));
	// We only add documentation, so don't bother if we already have
	// some.
...
// unparsed arguments for example. This function returns the default
// argument if it is available.
const Expr*
getDefaultArg(const ParmVarDecl* PVD)
{
	// Default argument can be unparsed or uninstantiated. For the
	// former we can't do much, as token information is only stored
	// in Sema and not attached to the AST node. For the latter
	// though, it is safe to proceed as the expression is still
	// valid.
	if (!PVD->hasDefaultArg() || PVD->hasUnparsedDefaultArg())
		return nullptr;
	return PVD->hasUninstantiatedDefaultArg() ? PVD->getUninstantiatedDefaultArg() : PVD->getDefaultArg();
}

HoverInfo::Param
toHoverInfoParam(const ParmVarDecl* PVD, const PrintingPolicy& PP)
{
	HoverInfo::Param Out;
...
void
fillFunctionTypeAndParams(HoverInfo& HI, const Decl* D, const FunctionDecl* FD, const PrintingPolicy& PP)
{
	...
	// We don't want any type info, if name already contains it.
	// This is true for constructors/destructors and conversion
	// operators.
	const auto NK = FD->getDeclName().getNameKind();

	if (NK == DeclarationName::CXXConstructorName || NK == DeclarationName::CXXDestructorName || NK == DeclarationName::CXXConversionFunctionName)
		return;
...
static llvm::FormattedNumber
printHex(const llvm::APSInt& V)
{
...
printExprValue(const Expr* E, const ASTContext& Ctx)
{
	// InitListExpr has two forms, syntactic and semantic. They are
	// the same thing (refer to a same AST node) in most cases. When
	// they are different, RAV returns the syntactic form, and we
	// should feed the semantic form to EvaluateAsRValue.
	...
	// Evaluating [[foo]]() as "&foo" isn't useful, and prevents us
...
// Seek the closest evaluable expression along the ancestors of node
// N in a selection tree. If a node in the path can be converted to
// an evaluable Expr, a possible evaluation would happen and the
// associated context is returned. If evaluation couldn't be done,
// return the node where the traversal ends.
PrintExprResult
printExprValue(const SelectionTree::Node* N, const ASTContext& Ctx)
{
	for (; N; N = N->Parent)
	{
		// Try to evaluate the first evaluatable enclosing
		// expression.
		if (const Expr* E = N->ASTNode.get<Expr>())
		{
			// Once we cross an expression of type 'cv void', the
			// evaluated result has nothing to do with our original
			// cursor position.
		...
		{
			// Refuse to cross certain non-exprs. (TypeLoc are OK as
			// part of Exprs). This tries to ensure we're showing a
			// value related to the cursor.
			break;
		}
	}
...
getHoverContents(const NamedDecl* namedDecl, const PrintingPolicy& printPolicy, const SymbolIndex* index, const syntax::TokenBuffer& tokenBuffer)
{
	...
	HI.LocalScope = getLocalScope(namedDecl);

	if (!HI.LocalScope.empty())
		HI.LocalScope.append("::");

	HI.Name				 = printName(Ctx, *namedDecl);
	const auto* CommentD = getDeclForComment(namedDecl);
	...
	{
		HI.TemplateParameters = fetchTemplateParameters(TD->getTemplateParameters(), printPolicy);
		namedDecl			  = TD;
	}
	else if (const FunctionDecl* FD = namedDecl->getAsFunction())
	{
		if (const auto* FTD = FD->getDescribedTemplate())
		{
			HI.TemplateParameters = fetchTemplateParameters(FTD->getTemplateParameters(), printPolicy);
			namedDecl			  = FTD;
		}
	}

	// Fill in types and params.
	if (const FunctionDecl* FD = getUnderlyingFunction(namedDecl))
		fillFunctionTypeAndParams(HI, namedDecl, FD, printPolicy);
	else if (const auto* VD = dyn_cast<ValueDecl>(namedDecl))
		HI.Type = printType(VD->getType(), Ctx, printPolicy);
	else if (const auto* TTP = dyn_cast<TemplateTypeParmDecl>(namedDecl))
		HI.Type = TTP->wasDeclaredWithTypename() ? "typename" : "class";
	else if (const auto* TTP = dyn_cast<TemplateTemplateParmDecl>(namedDecl))
		HI.Type = printType(TTP, printPolicy);
	else if (const auto* VT = dyn_cast<VarTemplateDecl>(namedDecl))
		HI.Type = printType(VT->getTemplatedDecl()->getType(), Ctx, printPolicy);
	else if (const auto* TN = dyn_cast<TypedefNameDecl>(namedDecl))
		HI.Type = printType(TN->getUnderlyingType().getDesugaredType(Ctx), Ctx, printPolicy);
...
getPredefinedExprHoverContents(const PredefinedExpr& PE, ASTContext& Ctx, const PrintingPolicy& PP)
{
	HoverInfo HI;
...
HoverInfo
evaluateMacroExpansion(unsigned int SpellingBeginOffset, unsigned int SpellingEndOffset, llvm::ArrayRef<syntax::Token> Expanded, ParsedAST& AST)
{
	...
	// where L_BRACKET and R_BRACKET expand to
	...
	if (!StartNode)
		return {};
	// If the common ancestor is partially selected, do evaluate if
	// it has no children, thus we can disallow evaluation on
	// incomplete expression. For example, #define PLUS_2 +2 40
	// PL^US_2 In this case we don't want to present 'value: 2' as
	// PLUS_2 actually expands to a non-value rather than a binary
	// operand.
...
getHoverContents(const DefinedMacro& Macro, const syntax::Token& Tok, ParsedAST& AST)
{
	...
	// Ensure that EndLoc is a valid offset. For example it might
	// come from preamble, and source file might've changed, in such
	// a scenario EndLoc still stays valid, but getLocForEndOfToken
	// will fail as it is no longer a valid offset. Note that this
	// check is just to ensure there's text data inside the range.
	// It will still succeed even when the data inside the range is
	// irrelevant to macro definition.
	if (SM.getPresumedLoc(EndLoc, /*UseLineDirectives=*/false).isValid())
	{
		...
		if (!Invalid)
		{
			...
			if (EndOffset <= Buffer.size() && StartOffset < EndOffset)
				HI.Definition = ("#define " + Buffer.substr(StartOffset, EndOffset - StartOffset)).str();
		}
	}
	...
	if (auto Expansion = AST.getTokens().expansionStartingAt(&Tok))
	{
		...
		// For extremely long expansion text, it's not readable from
		// hover card anyway.
...
getThisExprHoverContents(const CXXThisExpr* CTE, ASTContext& ASTCtx, const PrintingPolicy& PP)
{
	...
	// For partial specialization class, origin `this` pointee type
	// will be parsed as `InjectedClassNameType`, which will ouput
	// template arguments like "type-parameter-0-0". So we retrieve
	// user written class type in this case.
	QualType PrettyThisType = ASTCtx.getPointerType(QualType(ClassType.getTypePtr(), OriginThisType.getCVRQualifiers()));
...
HoverInfo
getDeducedTypeHoverContents(QualType QT, const syntax::Token& Tok, ASTContext& ASTCtx, const PrintingPolicy& PP, const SymbolIndex* Index)
{
	HoverInfo HI;
...
bool
isLiteral(const Expr* E)
{
	// Unfortunately there's no common base Literal classes inherits
	// from (apart from Expr), therefore these exclusions.
	return llvm::isa<CompoundLiteralExpr>(E) || llvm::isa<CXXBoolLiteralExpr>(E) || llvm::isa<CXXNullPtrLiteralExpr>(E) || llvm::isa<FixedPointLiteral>(E) || llvm::isa<FloatingLiteral>(E) ||
		llvm::isa<ImaginaryLiteral>(E) || llvm::isa<IntegerLiteral>(E) || llvm::isa<StringLiteral>(E) || llvm::isa<UserDefinedLiteral>(E);
}

llvm::StringLiteral
getNameForExpr(const Expr* E)
{
	// FIXME: Come up with names for `special` expressions.
	//
	// It's an known issue for GCC5, https://godbolt.org/z/Z_tbgi.
	// Work around that by using explicit conversion constructor.
	//
	// TODO: Once GCC5 is fully retired and not the minimal
	// requirement as stated in `GettingStarted`, please remove the
	// explicit conversion constructor.
	return llvm::StringLiteral("expression");
}

void maybeAddCalleeArgInfo(const SelectionTree::Node* N, HoverInfo& HI, const PrintingPolicy& PP);

// Generates hover info for `this` and evaluatable expressions.
// FIXME: Support hover for literals (esp user-defined)
std::optional<HoverInfo>
getHoverContents(const SelectionTree::Node* N, const Expr* E, ParsedAST& AST, const PrintingPolicy& PP, const SymbolIndex* Index)
{
	...
	// For `this` expr we currently generate hover with pointee
	// type.
...
std::optional<HoverInfo>
getHoverContents(const Attr* A, ParsedAST& AST)
{
	HoverInfo HI;
		...
		if (Record && !Record->isInvalidDecl() && !Record->isDependentType())
		{
			...
			if (HI.Size)
			{
				...
				if (!Record->isUnion() && FD->getFieldIndex() + 1 < Layout.getFieldCount())
				{
					...
					if (NextOffset >= EndOfField) // next field could be a
												  // bitfield!
			...
			// Offset in a union is always zero, so not really
			// useful to report.
			if (Record->isUnion())
				HI.Offset.reset();
		}
...
// If N is passed as argument to a function, fill HI.CalleeArgInfo
// with information about that argument.
void
maybeAddCalleeArgInfo(const SelectionTree::Node* N, HoverInfo& HI, const PrintingPolicy& PP)
{
	...
	else if (const auto* CE = OuterNode.Parent->ASTNode.get<CXXConstructExpr>())
	{
	...
	// For non-function-call-like operators (e.g. operator+,
	// operator<<) it's not immediately obvious what the "passed as"
	// would refer to and, given fixed function signature, the value
	// would be very low anyway, so we choose to not support that.
	// Both variadic functions and operator() (especially relevant
	// for lambdas) should be supported in the future.
	...
	// [const-]reference. For this we need to walk up the AST from
	// the arg itself to CallExpr and check all implicit casts,
	...
	for (auto* CastNode = N->Parent; CastNode != OuterNode.Parent && !PassType.Converted; CastNode = CastNode->Parent)
	{
		if (const auto* ImplicitCast = CastNode->ASTNode.get<ImplicitCastExpr>())
		{
			switch (ImplicitCast->getCastKind())
			{
				...
				case CK_UncheckedDerivedToBase:
					// If it was a reference before, it's still a
					// reference.
					if (PassType.PassBy != HoverInfo::PassType::Value)
						PassType.PassBy = ImplicitCast->getType().isConstQualified() ? HoverInfo::PassType::ConstRef : HoverInfo::PassType::Ref;
					break;
				...
				case CK_NullToMemberPointer:
					// No longer a reference, but we do not show
					// this as type conversion.
		...
		else if (const auto* CtorCall = CastNode->ASTNode.get<CXXConstructExpr>())
		{
			// We want to be smart about copy constructors. They
			// should not show up as type conversion, but instead as
			// passing by value.
		...
		{
			// Can't bind a non-const-ref to a temporary, so has to
			// be const-ref
			PassType.PassBy = HoverInfo::PassType::ConstRef;
		}
		else
...
const NamedDecl*
pickDeclToUse(llvm::ArrayRef<const NamedDecl*> Candidates)
{
	if (Candidates.empty())
		return nullptr;
	...
	// One declaration in Candidates will refer to the using
	// declaration, which isn't really useful for Hover. So use the
	// other one, which in this example would be the actual
	// declaration of foo.
...
void
maybeAddSymbolProviders(ParsedAST& AST, HoverInfo& HI, include_cleaner::Symbol Sym)
{
	trace::Span Tracer("Hover::maybeAddSymbolProviders");

	llvm::SmallVector<include_cleaner::Header> RankedProviders = include_cleaner::headersForSymbol(Sym, AST.getPreprocessor(), &AST.getPragmaIncludes());
	...
	{
		if (P.kind() == include_cleaner::Header::Physical && P.physical() == SM.getFileEntryForID(SM.getMainFileID()))
	...
	HI.Provider = include_cleaner::spellHeader({ H, AST.getPreprocessor().getHeaderSearchInfo(), SM.getFileEntryForID(SM.getMainFileID()) });
}

// FIXME: similar functions are present in FindHeaders.cpp
// (symbolName) and IncludeCleaner.cpp (getSymbolName). Introduce a
// name() method into include_cleaner::Symbol instead.
std::string
...
std::optional<HoverInfo>
getHover(ParsedAST& AST, Position Pos, const format::FormatStyle& Style, const SymbolIndex* Index)
{
	...
	for (const auto& include : AST.getIncludeStructure().MainFileIncludes)
	{
		if (include.Resolved.empty() || include.HashLine != Pos.line)
			continue;
	...
	CharSourceRange highlightRange = tokensTouchingCursor.back().range(srcMgr).toCharRange(srcMgr);

	std::optional<HoverInfo> hoverInfo;
	...
	for (const auto& token : tokensTouchingCursor)
	{
		if (token.kind() == tok::identifier)
		{
			// Prefer the identifier token as a fallback highlighting range.
			highlightRange = token.range(srcMgr).toCharRange(srcMgr);

			if (auto macro = locateMacroAt(token, AST.getPreprocessor()))
			{
				HoverCountMetric.record(1, "macro");

				hoverInfo = getHoverContents(*macro, token, AST);

				if (auto DefLoc = macro->Info->getDefinitionLoc(); DefLoc.isValid())
				{
					include_cleaner::Macro IncludeCleanerMacro{ AST.getPreprocessor().getIdentifierInfo(token.text(srcMgr)), DefLoc };

					maybeAddSymbolProviders(AST, *hoverInfo, include_cleaner::Symbol{ IncludeCleanerMacro });
				}

				break;
			}
		}
		else if (token.kind() == tok::kw_auto || token.kind() == tok::kw_decltype)
		{
			HoverCountMetric.record(1, "keyword");

			if (auto Deduced = getDeducedType(AST.getASTContext(), token.location()))
			{
				hoverInfo	   = getDeducedTypeHoverContents(*Deduced, token, AST.getASTContext(), printPolicy, Index);
				highlightRange = token.range(srcMgr).toCharRange(srcMgr);
	...
	if (!hoverInfo)
	{
		auto Offset = srcMgr.getFileOffset(*curLoc);

		// Editors send the position on the left of the hovered
		// character. So our selection tree should be biased right.
		// (Tested with VSCode).
		SelectionTree ST = SelectionTree::createRight(AST.getASTContext(), tokens, Offset, Offset);

		if (const SelectionTree::Node* N = ST.commonAncestor())
		{
			// FIXME: Fill in HighlightRange with range coming from
			// N->ASTNode.
			auto Decls = explicitReferenceTargets(N->ASTNode, DeclRelation::Alias, AST.getHeuristicResolver());
			...
			if (const auto* DeclToUse = pickDeclToUse(Decls))
			{
				...
				if (!hoverInfo->Value)
					hoverInfo->Value = printExprValue(N, AST.getASTContext()).PrintedValue;

				maybeAddCalleeArgInfo(N, *hoverInfo, printPolicy);

				if (!isa<NamespaceDecl>(DeclToUse))
					maybeAddSymbolProviders(AST, *hoverInfo, include_cleaner::Symbol{ *DeclToUse });
			}
			else if (const Expr* E = N->ASTNode.get<Expr>())
			{
				HoverCountMetric.record(1, "expr");

				hoverInfo = getHoverContents(N, E, AST, printPolicy, Index);
			}
			else if (const Attr* A = N->ASTNode.get<Attr>())
			{
				HoverCountMetric.record(1, "attribute");

				hoverInfo = getHoverContents(A, AST);
			}
...
// If the backtick at `Offset` starts a probable quoted range, return
// the range (including the quotes).
...
void
parseDocumentationLine(llvm::StringRef Line, markup::Paragraph& Out)
{
	// Check if this is a @param line (must start at beginning)
	if (Line.starts_with("@param"))
	{
		...
		if (Line.starts_with("["))
		{
			...
			if (EndBracket != llvm::StringRef::npos)
			{
				auto DirBlock = Line.slice(1, EndBracket);

				Line = Line.drop_front(EndBracket + 1).ltrim();

				// Split DirBlock on commas into directions
				llvm::SmallVector<llvm::StringRef, 4> DirParts;

				DirBlock.split(DirParts, ',');

				for (llvm::StringRef DirSpec : DirParts)
				{
					auto Clean = DirSpec.trim();
					auto Parts = Clean.split(':');

					std::string DirName = Parts.first.str();

					directions.push_back(DirName);

					if (Parts.second == "optional")
						isOptional[DirName] = true;
				}
			}
		}
		...
		{
			ParamName	= Line.slice(0, FirstSpace).str();
			Description = Line.drop_front(FirstSpace + 1).trim().str();
		}
		else
		{
			ParamName = Line.str();
		...
		// Format: `param`: description [in, out (optional)]
		Out.appendCode(ParamName);
		Out.appendText(": ");
		Out.appendText(Description);
	...
	// Check for backtick code ranges
...
void
parseDocumentation(llvm::StringRef Input, markup::Document& Output)
{
	...
	for (std::tie(Line, Rest) = Input.split('\n'); !(Line.empty() && Rest.empty()); std::tie(Line, Rest) = Rest.split('\n'))
	{
		...
		// After a linebreak remove spaces to avoid 4 space markdown
		// code blocks.
...
llvm::raw_ostream&
operator<<(llvm::raw_ostream& OS, const HoverInfo::PrintedType& T)
{
	OS << T.Type;
...
llvm::raw_ostream&
operator<<(llvm::raw_ostream& OS, const HoverInfo::Param& P)
{
	if (P.Type)
		OS << P.Type->Type;
