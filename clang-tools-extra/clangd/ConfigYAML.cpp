...
bestGuess(llvm::StringRef Search, llvm::ArrayRef<llvm::StringRef> AllowedValues)
{
...
public:
	...
	parse(Fragment& F, Node& N)
	{
		...
		Dict.handle("Style", [&](Node& N)
					{ parse(F.Style, N); });
		Dict.handle("Diagnostics", [&](Node& N)
					{ parse(F.Diagnostics, N); });
		Dict.handle("Completion", [&](Node& N)
					{ parse(F.Completion, N); });
		Dict.handle("Hover", [&](Node& N)
					{ parse(F.Hover, N); });
		Dict.handle("InlayHints", [&](Node& N)
					{ parse(F.InlayHints, N); });
		Dict.handle("SemanticTokens", [&](Node& N)
					{ parse(F.SemanticTokens, N); });
		Dict.handle("C32", [&](Node& N)
					{ parse(F.C32, N); });
...
private:
	void
	parse(Fragment::IfBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::CompileFlagsBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::StyleBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::DiagnosticsBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::DiagnosticsBlock::ClangTidyBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::DiagnosticsBlock::IncludesBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::IndexBlock& F, Node& N)
	{
		...
		Dict.handle(
			...
			[&](Node& N)
			{
				...
				else if (N.getType() == Node::NK_Scalar || N.getType() == Node::NK_BlockScalar)
				{
	...
	void
	parse(Fragment::IndexBlock::ExternalBlock& F, Located<std::string> ExternalVal)
	{
		if (!llvm::StringRef(*ExternalVal).equals_insensitive("none"))
		{
			error("Only scalar value supported for External is 'None'", ExternalVal.Range);
			return;
		}
	...
	void
	parse(Fragment::IndexBlock::ExternalBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::CompletionBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::HoverBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::InlayHintsBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::SemanticTokensBlock& F, Node& N)
	{
	...
	void
	parse(Fragment::C32Block& F, Node& N)
	{
	...
	void
	parse(Fragment::C32Block::HoverBlock& F, Node& N)
	{
		DictParser Dict("Hover", this);
		...
		Dict.handle("ShowScope", [&](Node& N)
					{ F.ShowScope = boolValue(N, "ShowScope"); });
	...
	public:
		...
		void
		handle(llvm::StringLiteral Key, std::function<void(Node&)> Parse)
		{
	...
	private:
		void
		warnUnknownKeys(llvm::ArrayRef<Located<std::string>> UnknownKeys, const llvm::SmallSet<std::string, 8>& SeenKeys) const
		{
			...
			for (const Located<std::string>& UnknownKey : UnknownKeys)
				if (auto BestGuess = bestGuess(*UnknownKey, UnseenKeys))
					Outer->warning("Unknown " + Description + " key '" + *UnknownKey + "'; did you mean '" + *BestGuess + "'?", UnknownKey.Range);
				else
					Outer->warning("Unknown " + Description + " key '" + *UnknownKey + "'", UnknownKey.Range);
		}
	};

	// Try to parse a single scalar value from the node, warn on failure.
	std::optional<Located<std::string>>
	scalarValue(Node& N, llvm::StringRef Desc)
	{
	...
	boolValue(Node& N, llvm::StringRef Desc)
	{
	...
	uint32Value(Node& N, llvm::StringRef Desc)
	{
		if (auto Scalar = scalarValue(N, Desc))
		{
	...
	std::optional<std::vector<Located<std::string>>>
	scalarValues(Node& N)
	{
	...
	void
	error(const llvm::Twine& Msg, llvm::SMRange Range)
	{
		HadError = true;
		SM.PrintMessage(Range.Start, llvm::SourceMgr::DK_Error, Msg, Range);
	}
	void
	error(const llvm::Twine& Msg, const Node& N)
	{
	...
	void
	warning(const llvm::Twine& Msg, llvm::SMRange Range)
	{
		SM.PrintMessage(Range.Start, llvm::SourceMgr::DK_Warning, Msg, Range);
	}
	void
	warning(const llvm::Twine& Msg, const Node& N)
	{
...
std::vector<Fragment>
Fragment::parseYAML(llvm::StringRef YAML, llvm::StringRef BufferName, DiagnosticCallback Diags)
{
	...
	for (auto& Doc : llvm::yaml::Stream(*Buf, *SM))
	{
		if (Node* N = Doc.getRoot())
		{
			...
			SM->PrintMessage(Fragment.Source.Location, llvm::SourceMgr::DK_Note, "Parsing config fragment");
			if (Parser(*SM).parse(Fragment, *N))
				Result.push_back(std::move(Fragment));
		}
	}
	SM->PrintMessage(SM->FindLocForLineAndColumn(SM->getMainFileID(), 0, 0), llvm::SourceMgr::DK_Note, "Parsed " + llvm::Twine(Result.size()) + " fragments from file");
